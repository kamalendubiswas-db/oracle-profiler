...
SELECT TO_CHAR(mtime,'YYYY/MM/DD') mtime,
	...
	FROM
		...
		FROM dba_hist_active_sess_history ash,
		  (select stat_name,sum(to_number(value)) as core_nb from gv$osstat where stat_name='NUM_CPUS' group by stat_name ) cpu
		WHERE
		  1=1
		  -- and sample_time > sysdate - 30
		GROUP BY ash.instance_number,ash.dbid,cpu.core_nb,
		  TO_CHAR(sample_time,'YYYY-MM-DD HH24'),
		  session_state,
		  cpu.core_nb
		) q,
		(select dbid,name from v$database) cont
	      WHERE cont.dbid=q.dbid
