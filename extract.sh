...
echo -e "\n\n################################################################################################################"
echo "<<< WARNING >>>: never use pluggable database information for the following items "
echo "                 - always use the root container service name"
echo -e "################################################################################################################\n\n"
read -p "Oracle Database Service name as registred in the Oracle listener (CDB\$ROOT container for multi/single tenant database) : " ora_service
...
relative_script_cdb='scripts/main'
d=`date +%Y%m%d_%H%M`
